...
import { Smile, Frown, Angry, MapPin } from "lucide-react";
...
const SentimentAnalysis = ({ data = [], isLoading = false, className, friend }: SentimentAnalysisProps) => {
  ...
  return (
    <DataCard 
      title="Current Mood" 
      description={friend ? `How ${friend.name} feels today` : "How different places affect your mood"}
          ...
          <div className="flex items-start gap-3 p-3 rounded-md bg-primary/5 border border-primary/20">
            ...
            <div>
              <div className="text-sm font-medium">
                {friend ? `${friend.name}'s mood improves at ${topLocation.name}` : 
                  `Your mood tends to improve at ${topLocation.name}`}
              </div>
              <div className="text-xs text-muted-foreground">
                {topLocation.type} locations correlate with {Math.round(topLocation.score * 100)}% more positive emotions
              </div>
            </div>
          </div>
        )}
